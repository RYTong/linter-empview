...
let get_error_position = (sText, iStartIndex) => {
...
let new_slt_linter_msg = (editorPath, iLine, iCol, sMsg) => {
    let start = [iLine, 0];
    let stop = [
        iLine, iCol + 2
    ];
...
export default {
    lintSLT2(editorText, editorPath) {
        ...
        do {
            ...
            // console.log("pre loop:", iPreStartIndex, iPreEndIndex);
            iStartIndex = editorText.indexOf(sStartEdge, iStartIndex + 1);
            // console.log("iStartIndex:", iStartIndex);
            if ((iStartIndex == iPreEndIndex+1) && (iPreEndIndex !== 0)) {
              iStartIndex++;
              continue;
            }

            iEndIndex = editorText.indexOf(sEndEdge, iEndIndex + 1);
            // console.log("iEndIndex:", iEndIndex);
            if (iEndIndex+1 == iStartIndex) {
              iEndIndex++;
              continue;
            }
